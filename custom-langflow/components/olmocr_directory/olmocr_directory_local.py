"""
olmOCR Directory Processor - Local GPU

Process entire directories of PDFs and images using local NVIDIA GPU.
...
from langflow.io import BoolInput, IntInput, MessageTextInput, Output, StrInput
from langflow.schema import Data, Message


class OlmOCRDirectoryLocal(Component):
    """
    olmOCR - Directory Processor (Local GPU)
    ...
    display_name = "olmOCR - Directory Processor (Local GPU)"
    description = "Batch process directories of PDFs/images using local NVIDIA GPU"
    documentation = "https://github.com/allenai/olmocr"
    icon = "Cpu"
    name = "OlmOCRDirectoryLocal"
    ...
    def _get_gpu_info(self) -> str:
        ...
        try:
            cmd = [
                ...
                "--environment", "olmocr",
                "python", "-c",
                "import torch; print(f'{torch.cuda.get_device_name(0)} - {torch.cuda.get_device_properties(0).total_memory / (1024**3):.1f}GB VRAM') if torch.cuda.is_available() else print('No CUDA GPU')"
            ]
            ...
            return result.stdout.strip() if result.stdout else "Unknown GPU"
    ...
    def process_directory(self) -> Message:
        """Process all supported files in a directory using local GPU."""
        ...
        try:
            ...
            self.log("Starting olmOCR Directory Processor with Local GPU")
            ...
            if not self._check_cuda_available():
                error_msg = (
                    ...
                    "For macOS/Windows or systems without NVIDIA GPU, use 'olmOCR - Directory Processor' (external API) instead."
                )
            ...
            self.log(f"CUDA GPU detected: {gpu_info}")
            ...
            if self.workspace_directory and self.workspace_directory.strip():
                ...
                workspace = Path.cwd() / f"olmocr_workspace_local_{timestamp}"
                workspace.mkdir(parents=True, exist_ok=True)
            ...
            self.status = f"Processing {len(discovered_files)} file(s) with Local GPU..."
            ...
            # Create summary
            success_msg = (
                f"✓ Successfully processed {len(discovered_files)} file(s) using Local GPU\n"
